                ...
                if let Some(extension) = path.extension().and_then(OsStr::to_str) {
                    ...
                    if contex_lock_guard.file_formats.contains(extension) {
                        match contex_lock_guard.work_mode {
                            Mode::AddText => {
                                add_comment(&path, contex_lock_guard.text.clone())
                                    .expect("Write error");
                            }
                            Mode::RemoveText => unimplemented!(), // remove_text(
                                                                  //     &path,
...
#[test]
fn test_run() -> Result<()> {
    let config: CommentAdderConfig =
        <CommentAdderConfig as crate::configuration::traits::BaseConfig>::build_config()?;
    dbg!(&config);
    run(config)?;
    Ok(())
}
